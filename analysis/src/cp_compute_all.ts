...
async function main(dataPath:string="analysis/data/headers.txt", blocknum=2016){
    ...
    for (let index = 0; index < i-blocknum; index++) {
        ...
        for (let chekpoint_depth = 72; chekpoint_depth<=blocknum; chekpoint_depth++) {
            let estimated_depth = (1200+timestamps[index+chekpoint_depth]-timestamps[index])/600
            ...
            if (chekpoint_depth>=estimated_depth-allowance) {
                ...
                if (!consecutive_accept){
                    minimum_accept_depth = chekpoint_depth
    ...
    for (let index = 0; index < i-blocknum; index++) {
        for (let chekpoint_depth = 72; chekpoint_depth<=2016; chekpoint_depth++) {
        }
    }
    ...
    plot([not_accepted_txns_plot,timestamp_ratio_plot])
    console.log(`Failed blocks = ${minimum_accept_depths[0]}`)

    let success = 0;
    let chekpoint_depth = 0;
    successes.forEach((val)=>{
        if (chekpoint_depth>=72&&chekpoint_depth<=2016) {
            console.log(`One in ${Math.ceil(1/(1-val/(i-2016)))} transactions of checkpoint depth ${chekpoint_depth} will be rejected`);
            success+=val
        }
        chekpoint_depth++
    })
